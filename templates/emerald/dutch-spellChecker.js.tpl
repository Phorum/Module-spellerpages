...
function spellChecker( textObject ) {
	...
	this.popUpUrl = '{URL->HTTP_PATH}' +
                        '/mods/spellerpages/spellerpages/speller-dutch' +
			'/spellchecker.html';
...
function terminateSpell() {
	...
	var msg = "Spellingscontrole voltooid:\n\n";
	...
	if( numrepl == 0 ) {
		...
		if( !this.wordWin ) {
			...
			if( this.wordWin.totalMisspellings() ) {
				msg += "Er zijn geen woorden gewijzigd.";
			} else {
				msg += "Geen fouten gevonden.";
			}
		}
	} else if( numrepl == 1 ) {
		msg += "1 woord gewijzigd.";
	} else {
		msg += numrepl + " woorden gewijzigd.";
	}
	if( msg ) {
		msg += "\n";
		alert( msg );
...
/* phorum added */
    function run_spellerpages()
    {
        ...
        // If no subject input can be found, then do not complain.
        // This allows template authors to leave out the subject field.
        var input_subject = document.getElementById('phorum_subject')  || //5.1
	                    document.getElementById('subject');           //5.2
        if (input_subject) {
            spellerInputs.push(input_subject);
        }

        // We always expect a body to be available.
